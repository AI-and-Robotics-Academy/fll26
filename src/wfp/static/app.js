...
function selectDevice(id) {
  ...
  document.getElementById('temp').textContent = device.temperature + '°C'; document.getElementById('humidity').textContent = device.humidity + '%'; document.getElementById('air-humidity').textContent = device.air_humidity + '%'; document.getElementById('last-seen').textContent = device.last_seen;
...
const aiChat = document.getElementById('aiChat'); const chatMessages = document.getElementById('chatMessages'); const chatHistory = [];
function addChatMessage(role, content) { const message = document.createElement('div'); message.className = `chat-message ${role}`; message.textContent = content; chatMessages.appendChild(message); chatMessages.scrollTop = chatMessages.scrollHeight; return message; }
document.getElementById('aiToggle').addEventListener('click', () => { aiChat.classList.toggle('hidden'); if (!aiChat.classList.contains('hidden')) document.getElementById('chatInput').focus(); });
document.getElementById('aiClose').addEventListener('click', () => aiChat.classList.add('hidden'));
document.getElementById('chatForm').addEventListener('submit', async (event) => { event.preventDefault(); const input = document.getElementById('chatInput'); const question = input.value.trim(); if (!question) return; addChatMessage('user', question); chatHistory.push({role: 'user', content: question}); input.value = ''; input.disabled = true; const thinking = addChatMessage('assistant', 'Thinking…'); try { const response = await fetch('/api/chat', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({message: question, history: chatHistory.slice(0, -1)})}); const result = await response.json(); thinking.remove(); const reply = result.reply || result.error || 'I could not answer right now.'; addChatMessage('assistant', reply); if (result.reply) chatHistory.push({role: 'assistant', content: reply}); } catch { thinking.remove(); addChatMessage('assistant', 'I could not connect to FireWatch AI.'); } input.disabled = false; input.focus(); });
